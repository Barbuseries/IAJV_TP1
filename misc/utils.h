...
//returns a random integer between x and y, but thread safe
...
//returns a random double between zero and 1, but thread safe
inline double doubleRand(double min = 0, double max = 1) {
	thread_local std::mt19937 generator(std::random_device{}());
...
{
  return x + doubleRand()*(y-x);
}

//returns a random bool
inline bool   RandBool()
{
  if (doubleRand() > 0.5) return true;

  else return false;
}

//returns a random double in the range -1 < n < 1
inline double RandomClamped()    {return doubleRand() - doubleRand();}
